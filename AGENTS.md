...
> ⛔ **REGRA DE BLOQUEIO DE PUSH DE CORREÇÃO:** Se o Gabriel reportar que um *bug ainda persiste* ("ainda não está funcionando", "ainda não treme", etc.), o agente está **PROIBIDO** de fazer `git push` das novas tentativas de correção até que o Gabriel teste localmente e declare `"Ok, foi resolvido. Pode dar push."`.
